// Real PDF Data - synced with app.js
const pdfData = [
    {
        id: 1,
...
// Load PDF details from sessionStorage
function loadPDFDetails() {
    const pdfJSON = sessionStorage.getItem('selectedPDF');
    
    if (!pdfJSON) {
        // If no PDF is selected, redirect to home
        window.location.href = 'index.html';
        return;
    }

    currentPDF = JSON.parse(pdfJSON);

    // Update page elements
    document.getElementById('pdfTitle').textContent = currentPDF.title;
    document.getElementById('pdfDescription').textContent = currentPDF.description;
    const sizeEl = document.getElementById('pdfSize');
    const pagesEl = document.getElementById('pdfPages');
    const pagesDetailEl = document.getElementById('pdfPagesDetail');
    const sizeDetailEl = document.getElementById('pdfSizeDetail');
    if (sizeEl) sizeEl.closest('div').classList.add('hidden');
    if (pagesEl) pagesEl.closest('div').classList.add('hidden');
    if (pagesDetailEl) pagesDetailEl.closest('div').classList.add('hidden');
    if (sizeDetailEl) sizeDetailEl.closest('div').classList.add('hidden');
    document.getElementById('categoryBadge').textContent = formatCategory(currentPDF.category);
    document.getElementById('pdfCategoryDetail').textContent = formatCategory(currentPDF.category);

    // Update page title
    document.title = `${currentPDF.title} - ExamPDFHub`;
}

// Load related PDFs (same category, excluding current)
function loadRelatedPDFs() {
    if (!currentPDF) return;

    const relatedPDFs = pdfData
        .filter(pdf => pdf.category === currentPDF.category && pdf.id !== currentPDF.id)
        .slice(0, 3);

    const container = document.getElementById('relatedPDFs');

    if (relatedPDFs.length === 0) {
        container.innerHTML = '<p class="text-gray-500 col-span-3 text-center">No related PDFs found</p>';
        return;
    }

    const cardsHTML = relatedPDFs.map(pdf => `
        <div class="pdf-card-small bg-gray-50 rounded-xl p-6 cursor-pointer" onclick="navigateToDetails(${pdf.id})">
            <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-700 rounded-lg flex items-center justify-center mb-4">
            ...
            <div class="flex items-center justify-between text-xs text-gray-500">
                <span><i class="fas fa-file-alt mr-1"></i> ${pdf.pages} pages</span>
                <span>${pdf.size}</span>
            </div>
        </div>
    `).join('');
...
// Navigate to another PDF details
...
// Open PDF in new tab
function openPDF() {
    if (currentPDF && currentPDF.driveLink) {
        window.open(currentPDF.driveLink, '_blank');
    }
}
...
function downloadPDF() {
    if (currentPDF && currentPDF.driveLink) {
        ...
        alert('Opening download link in new tab...');
    }
}